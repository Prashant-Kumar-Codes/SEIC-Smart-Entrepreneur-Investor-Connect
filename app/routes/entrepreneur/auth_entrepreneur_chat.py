...
def chat_page():
    ...
    cursor.execute("""
        SELECT DISTINCT
            ip.email AS investor_email, ip.full_name AS investor_name,
            ip.firm_name, ip.profile_image_url,
            ipp.preferred_sectors, ipp.investment_stage,
            ipp.min_ticket_size, ipp.max_ticket_size,
            ip.geography, ipp.investment_thesis, ipp.portfolio_highlights
        FROM investor_profiles ip
        LEFT JOIN investor_portfolio_profile ipp ON ip.email = ipp.email
        JOIN login_data ld ON ip.email = ld.email
        WHERE ip.email IN (
            SELECT investor_email FROM saved_investors WHERE entrepreneur_email = %s
        )
        ORDER BY ip.firm_name LIMIT 20
    """, (email,))
    ...
    return render_template(
        ...
        profile=profile, pitch=pitch, saved_investors=saved_investors,
        unread_msgs=get_unread_count(email), unread_notifs=get_notification_count(email),
    )
...
def get_chat_history():
    ...
    try:
        conn = get_request_conn(); cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute("""
            SELECT role, content, created_at FROM ai_chat_messages
            WHERE session_id = %s AND entrepreneur_email = %s AND mode = %s
            ORDER BY created_at ASC LIMIT 60
        """, (session_id, email, mode))
        messages = cursor.fetchall(); cursor.close()
        for m in messages:
...
def list_sessions():
    ...
    try:
        conn = get_request_conn(); cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute("""
            SELECT s.session_id, s.mode, s.investor_email, s.title, s.created_at, s.updated_at,
                (SELECT content FROM ai_chat_messages
                 WHERE session_id = s.session_id ORDER BY created_at DESC LIMIT 1) AS last_message
            FROM ai_chat_sessions s
            WHERE s.entrepreneur_email = %s AND s.mode = %s
            ORDER BY s.updated_at DESC LIMIT 20
        """, (email, mode))
        sessions = cursor.fetchall(); cursor.close()
        for s in sessions:
            for k in ('created_at', 'updated_at'):
                if s.get(k): s[k] = s[k].strftime('%Y-%m-%d %H:%M:%S')
...
def send_message():
    ...
    try:
        ...
        cursor.execute("""
            SELECT ld.email, ld.username, ep.startup_name, ep.stage, ep.industry, ep.location,
                   ep.bio, ep.focus_areas, ep.funding_amount, ep.funding_currency,
                   ep.use_of_funds, ep.team_size, ep.founded_year, ep.funding_required
            FROM login_data ld LEFT JOIN entrepreneur_profile ep ON ld.email = ep.email
            WHERE ld.email = %s
        """, (email,))
        ...
        if not session_id:
            import uuid; session_id = uuid.uuid4().hex
            title = _make_session_title(mode, user_message, profile, investor_email)
            cursor.execute("""
                INSERT INTO ai_chat_sessions (session_id, entrepreneur_email, mode, investor_email, title)
                VALUES (%s, %s, %s, %s, %s)
        ...
        cursor.execute("""
            ...
            WHERE session_id = %s ORDER BY created_at DESC LIMIT 16
        """, (session_id,))
        recent = list(reversed(cursor.fetchall()))

        if mode == 'guidance':
            system_prompt, contents = _build_guidance_prompt(profile, pitch, recent, user_message)
        else:
            cursor.execute("""
            ...
            system_prompt, contents = _build_intermediary_prompt(profile, pitch, investor, recent, user_message)

        client   = get_gemini_client()
        response = client.models.generate_content(
            model="gemini-2.5-flash", contents=contents,
            config=types.GenerateContentConfig(
                system_instruction=system_prompt, max_output_tokens=3000, temperature=0.45))
        ai_reply = response.text.strip()
        if not ai_reply: raise ValueError("Empty AI response")

        cursor.execute("INSERT INTO ai_chat_messages (session_id, entrepreneur_email, mode, role, content) VALUES (%s,%s,%s,'user',%s)",
            (session_id, email, mode, user_message))
        cursor.execute("INSERT INTO ai_chat_messages (session_id, entrepreneur_email, mode, role, content) VALUES (%s,%s,%s,'assistant',%s)",
            (session_id, email, mode, ai_reply))
        cursor.execute("UPDATE ai_chat_sessions SET updated_at = NOW() WHERE session_id = %s", (session_id,))
        conn.commit(); cursor.close()

        print(f"✅ AI chat [{mode}] for {email} — session {session_id[:8]}")
        return jsonify({'success': True, 'reply': ai_reply, 'session_id': session_id}), 200
    except Exception as e:
        error_msg = str(e).lower()
        print(f"❌ Chat send error: {e}")
        import traceback; traceback.print_exc()
        if any(x in error_msg for x in ["api key", "authentication", "401"]): msg = "AI service configuration error."
        elif any(x in error_msg for x in ["quota", "rate limit", "429"]): msg = "AI rate limit reached. Please try again shortly."
        elif any(x in error_msg for x in ["blocked", "safety"]): msg = "Request blocked by AI safety filters."
        else: msg = "AI is unavailable right now. Please try again."
...
def delete_session():
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    email = session.get('user_email')
    data = request.get_json(); session_id = data.get('session_id', '').strip()
    if not session_id:
        return jsonify({'success': False, 'message': 'No session_id.'}), 400
    try:
        conn = get_request_conn(); cursor = conn.cursor()
        cursor.execute("DELETE FROM ai_chat_sessions WHERE session_id = %s AND entrepreneur_email = %s", (session_id, email))
        conn.commit(); cursor.close()
...
def _profile_summary(profile, pitch):
    p = profile or {}; pc = pitch or {}
    lines = [
        f"Startup: {p.get('startup_name','N/A')}", f"Founder: {p.get('username','N/A')}",
        f"Industry: {p.get('industry','N/A')}", f"Stage: {p.get('stage','N/A')}",
        f"Location: {p.get('location','N/A')}", f"Team Size: {p.get('team_size','N/A')}",
        f"Founded: {p.get('founded_year','N/A')}",
        f"Funding Ask: {p.get('funding_currency','INR')} {p.get('funding_amount','N/A')}",
        f"Use of Funds: {p.get('use_of_funds','N/A')}", f"Bio: {p.get('bio','N/A')}", "",
        "Pitch Summary:",
        f"  Problem: {pc.get('problem','N/A')}", f"  Solution: {pc.get('solution','N/A')}",
        f"  Market: {pc.get('market','N/A')}", f"  Business Model: {pc.get('business_model','N/A')}",
        f"  Traction: {pc.get('traction','N/A')}", f"  The Ask: {pc.get('the_ask','N/A')}",
    ]
    return "\n".join(lines)
...
def _build_intermediary_prompt(profile, pitch, investor, recent, user_message):
    ...
    system_prompt = (
        ...
        "\n\nENTREPRENEUR PROFILE:\n" + _profile_summary(profile, pitch) +
        "\n\nINVESTOR PROFILE:\n"
        f"Name: {inv.get('full_name','N/A')}\nFirm: {inv.get('firm_name','N/A')}\n"
