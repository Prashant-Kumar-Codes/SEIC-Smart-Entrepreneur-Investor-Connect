...
(function () {
    ...
    window.handleMatchConnect = async function(btn) {
        const investorEmail = btn.dataset.investorEmail;
        const investorName  = btn.dataset.investorName;

        if (!investorEmail) {
            showToast('Invalid investor data.', 'error');
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                showToast(`Connection request sent to ${investorName}!`, 'success');
                btn.textContent    = '✓ Requested';
                btn.classList.add('match-connect-btn--sent');
                btn.disabled       = true;
            } else {
    ...
    window.handleMatchView = function(btn) {
        const investorEmail = btn.dataset.investorEmail;
        if (investorEmail) {
            // Navigate to investor public profile
            window.location.href = `/investor/profile/${encodeURIComponent(investorEmail)}`;
        } else {
            showToast('Profile view coming soon!', 'default');
        }
    };

    // Refresh match panel without full reload
    window.refreshMatches = async function() {
        try {
            const res  = await fetch('/api/matches/my', { headers: { 'Accept': 'application/json' } });
            const data = await res.json();
            if (data.success && Array.isArray(data.matches)) {
                const badge = document.querySelector('.matches-count-badge');
                if (badge) badge.textContent = `${data.matches.length} matched`;
                // Full re-render handled server-side on next page load
            }
        } catch (_) {}
    };
